...
export default async (req) => {
  ...
  const withinLimit = await checkRateLimit("photo-upload", ip, 30, 60 * 60 * 1000);
  if (!withinLimit) {
    return jsonResponse({ error: "Demasiadas subidas desde esta conexión. Inténtalo más tarde." }, 429);
  }
